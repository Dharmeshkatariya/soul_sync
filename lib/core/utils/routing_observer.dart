...
class RoutingObserver extends GetObserver {
  ...
  void didPush(Route route, Route? previousRoute) {
    Common().printLog("GetObserver", 'route:${route.settings.name}  previousRoute:${previousRoute?.settings.name}');
  ...
  void didReplace({Route? newRoute, Route? oldRoute}) {
    Common().printLog("GetObserver", 'newRoute:${newRoute?.settings.name}  oldRoute:${oldRoute?.settings.name}');
