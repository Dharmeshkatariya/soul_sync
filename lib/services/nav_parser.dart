    ...
    if (description_shelf[0] == row.keys.first.toString()) {
      ...
      for (var result in results['contents']) {
        ...
        if (data != null) {
          var pageType = nav(
            data,
            n_title + navigation_browse + page_type,
            noneIfAbsent: true,
            funName: "mixed1",
          );
          if (pageType == null) {
  ...
  Map<String, dynamic> song = {
    ...
    'videoId':
        nav(
          ...
          noneIfAbsent: true,
          funName: "parseSongFlat",
        ) ??
        nav(
          columns[0],
          text_run + navigation_browse_id,
          noneIfAbsent: true,
          funName: "parseSongFlat",
        ),
...
List<dynamic> parsePlaylistItems(
  ...
  for (dynamic result in results) {
    ...
    String? title = getItemCustomTextView(data, 0);
...
Map<String, dynamic>? parseSongAlbum(Map<String, dynamic> data, int index) {
  ...
  if (flexItem.isNotEmpty) {
    return {
      'name': getItemCustomTextView(data, index),
...
Map<String, dynamic> parseSongMenuTokens(Map<String, dynamic> item) {
  ...
  Map<String, dynamic> libraryAddToken = nav(toggleMenu, [
    'defaultServiceEndpoint',
    ...feedback_token,
  ]);
  Map<String, dynamic> libraryRemoveToken = nav(toggleMenu, [
...
dynamic parseTopResult(
  ...
  String? resultType = getSearchResultType(
    nav(data, subtitle),
    searchResultTypes,
  );
  ...
  if (resultType == 'artist') {
    ...
    Map<String, dynamic> artistInfo = parseSongRuns(
      nav(data, ['title', 'runs']),
    );
...
dynamic parseSearchResult(
  ...
  resultType = ((resultType == null)
      ? getSearchResultType(getItemCustomTextView(data, 1), searchResultTypes)
      : resultType)!;
  searchResult['resultType'] = resultType;

  if (resultType != 'artist') {
    searchResult['title'] = getItemCustomTextView(data, 0);
  }

  if (resultType == 'artist') {
    searchResult['artist'] = getItemCustomTextView(data, 0);
    final list =
    ...
    searchResult['type'] = getItemCustomTextView(data, 1);
    ...
    searchResult['itemCount'] = nav(flexItem, [
      defaultOffset + (hasAuthor ? 2 : 0),
      'text',
    ]).split(' ')[0];
    searchResult['description'] = hasAuthor
    ...
    if (browseId == null) {
      ...
      if (flexItems[0] != null) {
        searchResult['videoId'] =
            nav(flexItems[0][0], navigation_video_id) ??
...
MediaItem? parseChartsTrending(dynamic data) {
  ...
  final video = {
    ...
    'videoId':
        nav(flex_0, text_run + navigation_video_id) ??
